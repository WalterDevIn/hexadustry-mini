...
import { axialToPixel, generateVisibleHexes, hexCorner, HEX_DIRECTIONS } from "../hex/hexMath.js";
...
const ROCK_CLUSTER_FOOTPRINTS = {
  ...
  huge: [{ q: 0, r: 0 }, { q: 1, r: 0 }, { q: 1, r: -1 }, { q: 0, r: -1 }, { q: -1, r: 0 }, { q: -1, r: 1 }, { q: 0, r: 1 }],
};

function rgba(color, alpha) { return `rgba(${color[0]}, ${color[1]}, ${color[2]}, ${alpha})`; }
function easeOutCubic(t) { return 1 - (1 - t) ** 3; }
function makeRelativeKey(q, r) { return `${q},${r}`; }
function makeHexKeyFromHex(hex) { return makeRelativeKey(hex.q, hex.r); }
function getWallFootprint(source) { return source.footprint ?? [{ q: 0, r: 0 }]; }
...
function insetSegmentsToward(segments, target, insetPixels) {
  return segments.map((segment) => ({ start: insetPointToward(segment.start, target, insetPixels), end: insetPointToward(segment.end, target, insetPixels) }));
}
...
function strokeCornerSegments(ctx, segments, cornerRatio) {
  for (const segment of segments) {
    const startPieceEnd = { x: segment.start.x + (segment.end.x - segment.start.x) * cornerRatio, y: segment.start.y + (segment.end.y - segment.start.y) * cornerRatio };
    const endPieceStart = { x: segment.end.x + (segment.start.x - segment.end.x) * cornerRatio, y: segment.end.y + (segment.start.y - segment.end.y) * cornerRatio };
    ctx.beginPath();
    ctx.moveTo(segment.start.x, segment.start.y);
...
function getInnerWallInset(size) { return Math.min(5.5, Math.max(3.5, size * 0.18)); }
...
function getRockClusterStyle(naturalBlockType, alpha) {
  if (naturalBlockType === "dense-rock") return { fill: `rgba(255, 255, 255, ${0.08 * alpha})`, stroke: `rgba(255, 255, 255, ${0.84 * alpha})`, lineWidth: 2.1 };
...
function canPlaceHugeRockVisualCluster(mapWorld, candidate) { return canPlaceRockVisualCluster(mapWorld, candidate, ROCK_CLUSTER_FOOTPRINTS.huge, candidate.naturalBlockType); }
...
function drawBuilding(ctx, building, size, origin, gameState) {
  ...
  if (building.type === "core") {
    ...
    ctx.beginPath(); ctx.moveTo(-radius, -radius * 0.45); ctx.lineTo(radius, 0); ctx.lineTo(-radius, radius * 0.45); ctx.closePath(); ctx.stroke();
  } else if (building.type === "turret") {
    ctx.beginPath(); ctx.arc(0, 0, radius * 0.75, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(radius * 1.05, -radius * 0.25); ctx.stroke();
...
function drawPendingConstruction(ctx, construction, size, origin) {
  ...
  ctx.save(); ctx.translate(center.x, center.y); drawQueuedBuildPreviewShape(ctx, construction, size);
  if (progress > 0) drawAnimatedWallShape(ctx, construction, size, BUILD_ANIMATION_YELLOW, 0.34, 0.16 + easeOutCubic(progress) * 0.84);
  ctx.restore();
}
function drawPendingDeconstruction(ctx, deconstruction, size, origin) {
  ...
  ctx.save(); ctx.translate(center.x, center.y); drawAnimatedWallShape(ctx, deconstruction, size, BUILD_ANIMATION_RED, 0.42, Math.max(0.06, 1 - easeOutCubic(progress) * 0.94)); ctx.restore();
}
function drawBuildPreview(ctx, gameState, size, origin) {
  ...
  ctx.save(); ctx.translate(center.x, center.y); drawQueuedBuildPreviewShape(ctx, { footprint }, size); ctx.restore();
}
...
function getPlayerEntityAlpha(gameState, team) { if (team?.id !== "player") return 1; return 0.18 + getSpawnProgress(gameState) * 0.82; }
function drawUnitTurret(ctx, transform, turret, screenOrigin, alpha = 1) {
  ...
  ctx.save(); ctx.globalAlpha = alpha; ctx.translate(x, y); ctx.rotate(worldRotation);
  ctx.strokeStyle = "rgba(255, 236, 126, 0.96)"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-rearLength, 0); ctx.lineTo(frontLength, 0); ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, 2.4, 0, Math.PI * 2); ctx.fillStyle = "rgba(255, 236, 126, 0.9)"; ctx.fill(); ctx.restore();
}
function drawTriangleEntity(ctx, entityId, ecsWorld, screenOrigin, gameState) {
  ...
  ctx.save(); ctx.globalAlpha = entityAlpha; ctx.translate(x, y); ctx.rotate(transform.rotation);
  ctx.strokeStyle = renderable.stroke ?? DEFAULT_RENDER_COLORS.stroke; ctx.fillStyle = renderable.fill ?? DEFAULT_RENDER_COLORS.fill; ctx.lineWidth = renderable.lineWidth;
  if (renderable.equilateral) drawEquilateralTriangle(ctx, radius);
  else { ctx.beginPath(); ctx.moveTo(radius, 0); ctx.lineTo(-radius * 0.78, -radius * 0.66); ctx.lineTo(-radius * 0.46, 0); ctx.lineTo(-radius * 0.78, radius * 0.66); ctx.closePath(); }
  ctx.fill(); ctx.stroke();
  if (!renderable.equilateral) { ctx.beginPath(); ctx.moveTo(-radius * 0.24, 0); ctx.lineTo(radius * 0.58, 0); ctx.stroke(); }
  ctx.restore();
  if (turret) drawUnitTurret(ctx, transform, turret, screenOrigin, entityAlpha);
  if (renderable.showLabel !== false) { ctx.save(); ctx.globalAlpha = entityAlpha; ctx.font = "11px Courier New"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillStyle = renderable.labelColor ?? DEFAULT_RENDER_COLORS.label; ctx.fillText(renderable.label, x, y + radius + 14); ctx.restore(); }
  if (team?.id === "player") { ctx.save(); ctx.globalAlpha = entityAlpha; ctx.beginPath(); ctx.arc(x, y, radius + 8, 0, Math.PI * 2); ctx.strokeStyle = renderable.aura ?? DEFAULT_RENDER_COLORS.aura; ctx.lineWidth = 1; ctx.stroke(); ctx.restore(); }
}
function drawCircleEntity(ctx, entityId, ecsWorld, screenOrigin) {
  ...
  const x = screenOrigin.x + transform.x;
  const y = screenOrigin.y + transform.y;
  ctx.save(); ctx.translate(x, y); ctx.rotate(transform.rotation); ctx.strokeStyle = renderable.stroke ?? DEFAULT_RENDER_COLORS.stroke; ctx.fillStyle = renderable.fill ?? DEFAULT_RENDER_COLORS.fill; ctx.lineWidth = renderable.lineWidth;
  ctx.beginPath(); ctx.arc(0, 0, radius, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(radius * 0.9, 0); ctx.stroke(); ctx.restore();
  ctx.save(); ctx.font = "10px Courier New"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillStyle = renderable.labelColor ?? DEFAULT_RENDER_COLORS.label; ctx.fillText(renderable.label, x, y + radius + 12); ctx.restore();
}
function drawLineEntity(ctx, entityId, ecsWorld, screenOrigin) {
  ...
  const x = screenOrigin.x + transform.x;
  const y = screenOrigin.y + transform.y;
  ctx.save(); ctx.translate(x, y); ctx.rotate(transform.rotation); ctx.strokeStyle = renderable.stroke ?? DEFAULT_RENDER_COLORS.stroke; ctx.lineWidth = renderable.lineWidth; ctx.beginPath(); ctx.moveTo(-renderable.length * 0.5, 0); ctx.lineTo(renderable.length * 0.5, 0); ctx.stroke(); ctx.restore();
}
function drawEntitiesOnLayer(ctx, ecsWorld, layerId, origin, gameState) {
  ...
  for (const entityId of lineEntities) if (ecsWorld.components.mapLayer.get(entityId).id === layerId) drawLineEntity(ctx, entityId, ecsWorld, origin);
  const triangleEntities = queryEntities(ecsWorld, ["transform", "triangleRenderable", "mapLayer"]);
  for (const entityId of triangleEntities) if (ecsWorld.components.mapLayer.get(entityId).id === layerId) drawTriangleEntity(ctx, entityId, ecsWorld, origin, gameState);
  const circleEntities = queryEntities(ecsWorld, ["transform", "circleRenderable", "mapLayer"]);
  for (const entityId of circleEntities) if (ecsWorld.components.mapLayer.get(entityId).id === layerId) drawCircleEntity(ctx, entityId, ecsWorld, origin);
}
function drawScanlines(ctx, width, height) {
  ctx.save(); ctx.strokeStyle = "rgba(255, 255, 255, 0.035)"; ctx.lineWidth = 1;
  for (let y = 0; y < height; y += 4) { ctx.beginPath(); ctx.moveTo(0, y + 0.5); ctx.lineTo(width, y + 0.5); ctx.stroke(); }
  ctx.restore();
}
function getCameraTarget(gameState) { return gameState.ecsWorld.components.transform.get(gameState.playerEntityId) ?? { x: 0, y: 0 }; }
